...
# Paths
# -----------------------------
setwd('/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/TCR')
load.path <- "/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/saved_R_data/"
in.path <- "/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/Ben_PARSE_TB_Data/VDJ_Output/combined"
source_subdir <- "TCR_filtered"   # <-- strictly use filtered

# -----------------------------
# Step 1: Read ONLY filtered barcode_report.tsv into a list
# -----------------------------
# discover top-level sample_* dirs
sample_dirs <- list.dirs(in.path, full.names = TRUE, recursive = FALSE) %>%
  keep(~ str_detect(basename(.x), "^sample_\\d+$"))

stopifnot(length(sample_dirs) > 0)

# read barcode_report.tsv from TCR_filtered only
contig_raw_list <- map(sample_dirs, function(sd) {
  fp <- file.path(sd, source_subdir, "barcode_report.tsv")
  if (!file.exists(fp)) {
    stop("Missing filtered file: ", fp,
         "\nEach sample must have ", source_subdir, "/barcode_report.tsv")
  }
  read_tsv(fp, show_col_types = FALSE)
})
names(contig_raw_list) <- basename(sample_dirs)  # e.g., "sample_1273088"


# -----------------------------
# Step 2: Convert into scRepertoire contig list (ParseBio)
# -----------------------------
contig.list <- loadContigs(input = contig_raw_list, format = "ParseBio")

# combine across samples for downstream scRepertoire workflows
combined.TCR.PARSE <- combineTCR(
  contig.list,
...
# Extract sample IDs from combined object (strip "sample_" prefix)
sample_ids <- names(combined.TCR.PARSE) %>% sub("^sample_", "", .)

# --- Make IGRA vector aligned to samples ---
igra_vec <- igra_df %>%
...
# --- Add IGRA status ---
combined.TCR.PARSE <- addVariable(
  combined.TCR.PARSE,
  variable.name = "IGRA_status",
  variables = igra_vec
)
...
setwd('/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/TCR/Clonal_Visualizations'
)


clonalQuant(combined.TCR.PARSE, 
            cloneCall="strict", 
            chain = "both", 
            group.by = 'IGRA_status',
            scale = FALSE)
ggsave('Unique_Clone_TRAB_IgraposvsNeg.png',width=24,height=12)
...
clonalQuant(combined.TCR.PARSE, 
            ...
            chain = "both", 
            scale = FALSE)
ggsave('Clones_Strict_TRAB_raw.png',width=24,height=12)
...
clonalQuant(combined.TCR.PARSE, 
            ...
            chain = "both", 
            scale = TRUE)
ggsave('Clones_Strict_TRAB_scaled.png',width=24,height=12)



### Clonal Abundance
clonalAbundance(combined.TCR.PARSE, 
                cloneCall = "strict", 
                scale = FALSE)
...
clonalAbundance(combined.TCR.PARSE, 
                cloneCall = "strict", 
                scale = TRUE)
ggsave('Clonal_Abundance_TRAB_scaled.png',width=16,height=12)



### Clonal Length
clonalLength(combined.TCR.PARSE, 
             cloneCall="aa", 
             chain = "both") 
...
clonalHomeostasis(combined.TCR.PARSE, 
                  cloneCall = "strict",
...
######################################## CD3 Composition ##################################
setwd('/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/TCR/CD3_Composition')

percentAA(combined.TCR.PARSE, 
          chain = "TRA", 
          aa.length = 20)
...
percentAA(combined.TCR.PARSE, 
          chain = "TRB", 
          aa.length = 20)
ggsave('Percent_AA_TRAB.png',width=26,height=24)



###
positionalEntropy(combined.TCR.PARSE, 
                  chain = "both", 
                  aa.length = 20)
...
percentKmer(combined.TCR.PARSE, 
            cloneCall = "aa",
            chain = "TRA", 
            motif.length = 3, 
            top.motifs = 25)
