...
for (sample_path in sample_dirs) {
  ...
  if (!file.exists(file.path(dge_path, "count_matrix.mtx"))) {
    warning(paste("Skipping", sample_name, "- missing DGE_filtered data"))
    next
  }
...
# Calculate hemoglobin content (genes starting with HB, excluding HBP manually)
...
# Define output path
save_path <- "/home/akshay-iyer/Documents/Ben_TB_HIV_Mothers/saved_R_data/seu_parsef_merged.rds"
